...
\cilk{cilk} int count_matches (Tree *t) \ttlbrace \\
  \hib{match (} t \hib{) \ttlbrace} \\
    ...
    \hib{\ttrbrace ;}  \\
    \hib{Leaf(\regex{/foo[1-9]+/}): \ttlbrace} return 1; \hib{\ttrbrace ;} \\
    \hib{Leaf(_): \ttlbrace} return 0; \hib{\ttrbrace ;} \\
\ttrbrace
\end{tabbing}
\end{alltt}
\end{frame}

%    \hib{Leaf(str): \ttlbrace} return ( str \regex{=~ /foo[1-9]+/} ) ? 1 : 0; \hib{\ttrbrace ;} \\  \hib{\ttrbrace}  \\
...
\bigskip
See \code{intro/exprIntro/AbstractSyntax.sv}
\end{frame}
...
\biA
 ...
 \x We see this in \code{intro/exprIntro/Inline.sv}
\ei
\end{frame}
...
\biA
 ...
 \x See \code{intro/exprIntro/Inline.sv} again.
\ei
\end{frame}
...
\x This requires a bit of refactoring - \code{extensible/expr}
\ei
\end{frame}
...
\beA
\x Refactor \code{intro/exprIntro}, creating \code{extensible/expr}
\x Create the \code{implies} construct in another extension
\x How does \code{implies} know about inlining? 
...
\hig{independently-developed} language extensions. 
